...
/**
 * Tracks rolling stats for PoiCompetitorScan caching.
 */
public class PoiCompetitorCacheTracker {
    ...
    public PoiCompetitorCacheTracker(Yasui plugin, YasuiConfig config) {
        this.plugin = plugin;
        this.config = config;
    }
    ...
    public int getCacheSize() {
        return PoiCompetitorNmsHook.getCacheSize();
    }

    private void flushStats() {
        if (!config.isVillagerPOIEnabled() || !config.isPoiCompetitorCacheEnabled()) {
            return;
        }
        updateRollingBuckets(System.currentTimeMillis());
        long[] stats = PoiCompetitorNmsHook.drainStats();
