...
/**
 * Tracks rolling stats for PoiAccess lookup caching.
 */
public class PoiLookupCacheTracker {
    ...
    public PoiLookupCacheTracker(Yasui plugin, YasuiConfig config) {
        this.plugin = plugin;
        this.config = config;
    }
    ...
    public int getCacheSize() {
        return PoiLookupNmsHook.getCacheSize();
    }

    private void flushStats() {
        if (!config.isVillagerPOIEnabled() || !config.isPoiLookupCacheEnabled()) {
            return;
        }
        updateRollingBuckets(System.currentTimeMillis());
        long[] stats = PoiLookupNmsHook.drainStats();
