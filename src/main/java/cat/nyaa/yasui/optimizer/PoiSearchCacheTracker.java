...
/**
 * Tracks rolling stats for AcquirePoi search caching.
 */
public class PoiSearchCacheTracker {
    ...
    public PoiSearchCacheTracker(Yasui plugin, YasuiConfig config) {
        this.plugin = plugin;
        this.config = config;
    }
    ...
    public RollingStats getRollingStats() {
        ...
        for (int i = 0; i < rollingHits.length; i++) {
            hits += rollingHits[i];
            misses += rollingMisses[i];
            stores += rollingStores[i];
        }
        return new RollingStats(hits, misses, stores);
    }

    public int getCacheSize() {
        return PoiSearchNmsHook.getCacheSize();
    }

    private void flushStats() {
        if (!config.isVillagerPOIEnabled() || !config.isAcquirePoiCacheEnabled()) {
            return;
        }
        updateRollingBuckets(System.currentTimeMillis());
        long[] stats = PoiSearchNmsHook.drainStats();
